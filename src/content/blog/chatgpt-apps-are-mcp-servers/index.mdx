...
Nice one, Simone. So, let's talk about these [ChatGPT](https://chatgpt.com) apps that under the hood are nothing more than [MCP](https://modelcontextprotocol.io) servers. We've talked about this a lot in recent months. MCP is a protocol for exposing resources, tools, and prompts.

If you go into [OpenAI](https://openai.com)'s documentation they tell you an app is made of three pieces. But the MCP server is actually the only real piece. The UI widgets, the graphical interface that pops up while you use the app inside ChatGPT (say Google Maps, or the product list) are actually an output of the MCP server. And then they say the third part is the model. That's not true: the model is theirs, and their model, GPT, is the one that decides which resources of your app (which is an MCP server) to use.
...
As for the [Cheshire Cat](https://cheshirecat.ai), I'm adding this in. Not all contributors agree, because I asked around, and they're still a bit suspicious of these protocols since they're still unstable, still new. On one hand they're right, but it's also true that MCP has by now established itself and a minimum of support has to be offered. On A2A there are still doubts, even though in my view it's a much more freewheeling protocol, much more open web, and nothing stops you from using MCP servers inside your A2A agent.
